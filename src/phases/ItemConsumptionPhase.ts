...
export class ItemConsumptionPhase extends Phase {
  ...
  constructor(world: World, player: Player, tabCompleter?: TabCompleter) {
    ...
    this.potionItems = allItems.filter(item => item instanceof Potion) as Potion[];
  }
  ...
  async initialize(): Promise<void> {
    ...
    if (this.potionItems.length === 0) {
      Display.printError('No potions available');
      Display.newLine();
  ...
  async startInputLoop(): Promise<CommandResult | null> {
    ...
    const choices: ListItem[] = this.potionItems.map((item, index) => ({
    ...
    const list = new ScrollableList(choices, {
      ...
      onSelectionChange: item => {
        const selectedItem = this.potionItems[item.value as number];
    ...
    const selectedItem = this.potionItems[selectedIndex as number];
   ...
   * アイテム情報をフォーマットする
   */
  private formatItemInfo(item: Potion): string {
   ...
   * 選択されたアイテムの詳細情報を表示する
   */
  private displaySelectedItemDetails(item: Potion): void {
    Display.newLine();
