...
export class BattlePhase extends Phase {
  ...
  private startTurn(): PhaseResult {
    ...
    const actor = this.battle.getCurrentTurnActor();
  ...
  private startPlayerTurn(): PhaseResult {
    ...
    this.selectedSkills = [];
    this.actionPoints = this.battle.calculatePlayerActionPoints();

    this.output(`===== Turn ${this.battle.currentTurn} - Your turn =====`);
  ...
  private executeEnemyTurn(): PhaseResult {
    ...
    this.output(`===== Turn ${this.battle.currentTurn} - Enemy turn =====`);

    const result = this.battle.enemyAction();
  ...
  private async confirmAndExecuteSkills(): Promise<PhaseResult> {
    ...
    const validationError = this.battle.validateSelectedSkills(skills);
    if (validationError) {
    ...
    // スキル実行
    const turnResult = this.battle.playerUseMultipleSkills(this.selectedSkills);
  ...
  private endBattle(victory: boolean): PhaseResult {
    ...
    if (victory) {
      ...
      // ドロップアイテム処理
      const drops = this.battle.calculateDrops();
      if (drops.length > 0) {
