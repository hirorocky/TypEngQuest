...
func TestSceneRouting(t *testing.T) {
	...
	for _, required := range requiredScenes {
		found := false
		...
		if !found {
			t.Errorf("必要なシーン '%s' が定義されていません", required)
		}
	}
}

func TestSaveTimingEvents(t *testing.T) {
	// セーブタイミングイベントの定義
	events := GetSaveEvents()

	// 必要なセーブイベントが全て定義されていること
	...
	for _, required := range requiredEvents {
		...
		if !found {
			t.Errorf("必要なセーブイベント '%s' が定義されていません", required)
		}
	}
}
...
func TestGameStateTransitions(t *testing.T) {
	...
	tests := []struct {
		...
		{SceneHome, SceneBattleSelect, true},
		{SceneHome, SceneAgentManagement, true},
		{SceneHome, SceneEncyclopedia, true},
		{SceneHome, SceneStatistics, true},
		{SceneHome, SceneSettings, true},
		{SceneBattleSelect, SceneBattle, true},
		{SceneBattle, SceneReward, true},
		{SceneReward, SceneHome, true},
		{SceneBattle, SceneHome, false}, // バトル中から直接ホームには戻れない
	}
	...
	for _, tt := range tests {
		t.Run(tt.from+"→"+tt.to, func(t *testing.T) {
			...
			if result != tt.allowed {
				t.Errorf("遷移 %s → %s: expected %v, got %v", tt.from, tt.to, tt.allowed, result)
			}
		})
	}
}
