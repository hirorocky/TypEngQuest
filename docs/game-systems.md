...
* **ステータス偏向の増幅**: アクセサリタイプ固有のステータス（攻撃型ならstrength、支援型ならwillpowerなど）を基礎値に対する割合で上昇させる。Grade 1では+8%程度から始まり、Grade 100では+35%付近まで伸長する。タイプが苦手とするステータスに対しては、初期段階でペナルティを設ける場合でも、グレードが高いほど緩和される。
  * 名前はアクセサリのタイプに由来する。
    ...
    * Bracelet
    * Earring
* **特殊効果の付与**: 戦闘や探索に影響を与える特殊効果（例: 「クリティカル発生時に追加MP回復」「タイピング制限時間+1秒」「状態異常耐性+40%」など）を付与する。全てのアクセサリは、グレードに関係なく特殊効果スロットを常時3つ保持し、合成で取得した候補効果から任意の3枠を選択して固定できる。
* **アクセサリの名前**: アクセサリタイプ x 特殊効果 x グレードで命名される。
  * 例: Cronus Glove 20 (攻撃型 x タイピング制限時間+1秒 x Grade 20)
...
アクセサリの合成は、同名アクセサリ2個を消費して特殊効果を再構成する手段である。合成処理は以下の手順で実施する。

1. **素材投入**: 同名アクセサリ2個（ベース＋素材）を投入する。
...
3. 高グレードアクセサリを素材として合成し、ベースのグレードを引き上げつつ、特殊効果候補から任意の3枠を厳選する（例: タイピング支援＋状態異常耐性＋クリティカル補助）。
4. Grade 100帯に到達したアクセサリは、シグネチャ補正を活かし、他装備・スキルとのシナジーを前提に最終調整を行う。

このフローにより、プレイヤーはワールドレベル進行と同調した段階的なグレード引き上げと、常時3枠の特殊効果マネジメントによってビルドの多様性をコントロールできる。

#### 実装メモ（2025-09-22）
- `AccessorySlotManager` がアクセサリスロット（初期1枠、キーアイテムで最大3枠）とグレード解禁／合成候補を一元管理する。
- `AccessoryCatalog` と `AccessoryNameGenerator` により、データ定義（`data/accessories/catalog.json`）から命名規則「タイプ＋ハイライト効果＋Grade番号」を自動適用する。
