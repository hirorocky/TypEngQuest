...
- **ダメージ計算**:
  - 基本ダメージ = (strength × 技倍率) - (敵防御力 × 0.5)
  - 最小ダメージは1
  - クリティカル時: ダメージ × 1.5
- **各ステータスの効果**:
  - **strength**: 攻撃スキルのダメージに影響
  - **willpower**: 回復スキル、デバフスキル、一時ステータス回復・継続速度に影響
  - **agility**: 行動順、1ターン行動ポイント、スキル回避率、逃走の成功率に影響
  - **fortune**: クリティカル率、アイテムドロップ率、スキルの特殊効果発生率に影響
- **クリティカル率**:
  - 基本クリティカル率 = 5 + (fortune / 15)%
  - 最大25%、最小5%
- **アイテムドロップ率**:
...
## 装備システム
- 装備アイテムはグレードがあり、グレードが高いほど強力なステータス、技を持つ
- 装備アイテムは個別のステータスを持つ:
  - attack: 攻撃力。プレイヤーのstrengthに加算される
  - defense: 防御力。プレイヤーのwillpowerに加算される
  - agility: 敏捷性。プレイヤーのagilityに加算される
  - fortune: 幸運。プレイヤーのfortuneに加算される
- 装備アイテムは1つにつき最大1つの技を持つことがある
  - 技名: 技の効果や条件を示す
  - 消費MP: 技を使用する際に消費するマナポイント
  - 成功率: accuracyかfortuneに依存
  - タイピング難易度: 技を使用するためのタイピングチャレンジの難易度
  - 技効果: タイピングチャレンジの結果に応じて発動する
    - 攻撃 (damage): 敵にダメージを与える
    - 回復 (heal): 自分のHPを回復する
    - 一時ステータス (temporary_status): バフ・デバフ・状態異常を付与する
      - バフ例: 攻撃力アップ、防御力アップ、速度アップ等
      - デバフ例: 攻撃力ダウン、防御力ダウン、速度ダウン等
      - 状態異常例: 毒（継続ダメージ）、麻痺（行動不能）、睡眠（行動不能）等
  - 各技は対象（target）を持つ: self（自分）、enemy（敵）、all（全体）
  - 基本技として`attack`があり、消費MPは0、タイピング難易度は最低、効果は敵にダメージを与える
- 装備効果は装備中の全アイテムステータスの合計、装備グレードの平均値（小数点切り捨て）がプレイヤーのレベルになる
- 使える技は装備中の各アイテムが持つ技+`attack`
- 装備アイテムは英単語になっており、同じ名前でも性能が異なるので、分けて管理する
- 装備できるのは最大で5単語で、有効な英文を構成する必要がある、文法的に間違っている場合は装備できない
...
- **精度評価**:
  ...
  - Good: 90%以上
  - Poor: 90%未満
- **総合評価**: 速度と精度の組み合わせで技の効果を決定
  - S + Perfect: 150%効果
  - A + Perfect/Great: 120%効果
  - B + Perfect/Great/Good: 100%効果
  - C + Any: 80%効果
  - F or Poor: 失敗（効果なし）
